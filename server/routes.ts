...
import { z } from "zod";
import { db } from "./db";
import { projects, paymentStages } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment-stages/:id/generate-link", requireAuth, async (req, res) => {
    try {
      const stageId = parseInt(req.params.id);
      ...
      const mockPaymentLink = `https://mercadopago.com/checkout/v2/redirect?preference_id=mock_${stageId}_${Date.now()}`;
      
      // Update the stage with the payment link
      const updated = await storage.updatePaymentStage(stageId, {
        paymentLink: mockPaymentLink,
        mercadoPagoId: `mock_${stageId}_${Date.now()}`
      });
